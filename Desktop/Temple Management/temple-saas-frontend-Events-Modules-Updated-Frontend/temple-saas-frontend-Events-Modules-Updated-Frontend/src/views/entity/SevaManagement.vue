...
const loadSevas = async () => {
  ...
  try {
    loading.value = true
    // Use the getEntityBookings method instead of fetchSevas
    const result = await sevaService.getEntityBookings(entityId)
    sevaList.value = result.data || []
...
// Keep the seva creation functionality intact
const editSeva = (seva) => {
  console.log('Editing seva:', seva)
  
  // Populate the form with the seva data - using availability_schedule for date
  sevaForm.value = {
    name: seva.name,
    type: seva.type,
    ...
    date: seva.availability_schedule || '', // Changed from date to availability_schedule
...
const saveSeva = async () => {
  ...
  try {
    ...
    sevaForm.value.entity_id = entityId
    
    // Create a formatted payload with all fields - map date to availability_schedule
    const payload = {
      name: sevaForm.value.name,
      type: sevaForm.value.type,
      description: sevaForm.value.description,
      price: parseFloat(sevaForm.value.price),
      duration: parseInt(sevaForm.value.duration),
      availability_schedule: sevaForm.value.date, // Changed from date to availability_schedule
      startTime: sevaForm.value.startTime,
      endTime: sevaForm.value.endTime,
      max_bookings_per_day: parseInt(sevaForm.value.max_bookings_per_day),
      is_active: sevaForm.value.is_active
    }
    
    console.log("Seva payload:", payload)
    ...
    if (editingSeva.value) {
      ...
      if (result.success) {
        ...
        showToast(result.message || 'Failed to create seva', 'error')
      }
    }
  } catch (error) {
