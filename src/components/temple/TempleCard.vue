<!-- src/components/temple/TempleCard.vue -->
<template>
    ...
    <div class="p-6">
      ...
      <div class="space-y-3 mb-6">
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="text-sm font-medium text-gray-900">{{ temple.category }}</p>
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            <p class="text-sm font-medium text-gray-900">{{ temple.address.city }}, {{ temple.address.state }}</p>
            <p class="text-xs text-gray-500">Location</p>
          </div>
        </div>

        <!-- Established Year -->
        <div class="flex items-center gap-3" v-if="temple.establishedYear">
          ...
          <div>
            <p class="text-sm font-medium text-gray-900">{{ temple.establishedYear }}</p>
      ...
      <!-- Contact Information -->
      <div class="border-t border-gray-100 pt-4 mb-4" v-if="temple.contact">
        <div class="grid grid-cols-1 gap-2">
          <div class="flex items-center gap-2" v-if="temple.contact.phone">
          ...
          <div class="flex items-center gap-2" v-if="temple.contact.email">
            <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <!-- Facilities -->
      <div class="mb-6" v-if="temple.facilities && temple.facilities.length > 0">
        <p class="text-sm font-medium text-gray-700 mb-2">Facilities:</p>
        <div class="flex flex-wrap gap-1">
          <span 
            v-for="facility in temple.facilities.slice(0, 3)" 
            :key="facility"
          ...
          <span 
            v-if="temple.facilities.length > 3"
            class="inline-flex items-center px-2 py-1 rounded-md text-xs font-medium bg-gray-100 text-gray-600"
          >
            +{{ temple.facilities.length - 3 }} more
          </span>
        </div>
      </div>

      <!-- Rejection Reason (if rejected) -->
      <div v-if="temple.status === 'REJECTED' && temple.rejection_reason" 
      ...
      <div class="flex flex-col sm:flex-row gap-2 pt-4 border-t border-gray-100">
        ...
        <button
          v-if="temple.status !== 'APPROVED' && showDeleteButton"
