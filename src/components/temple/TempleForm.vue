<!-- src/components/temple/TempleForm.vue -->
<template>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                ...
                type="text"
                class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors"
                placeholder="e.g., 1950 or 15th Century"
              />
            </div>
          </div>
        </div>
...
const handleSubmit = async () => {
  try {
    ...
    if (isEditing.value) {
      await templeStore.updateTemple(props.temple.id, formData)
    } else {
...
onMounted(() => {
  if (props.temple) {
    form.value = {
      ...props.temple,
      address: { ...props.temple.address },
      contact: { ...props.temple.contact },
      timings: { ...props.temple.timings },
      facilities: [...(props.temple.facilities || [])],
      documents: [...(props.temple.documents || [])]
    }
  }
})
